...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `${$$result.head += `<!-- HEAD_svelte-18krgu7_START -->${$$result.title = `<title>${escape(data.title)} | Spikey Sanju</title>`, ""}<meta name="${"description"}"${add_attribute("content", data.title, 0)}><!-- HEAD_svelte-18krgu7_END -->`, ""}
...
<article><h1>${escape(data.title)}</h1>
    ...
    return `<span class="${"inline-flex rounded-full bg-gray-100 px-2 text-xs font-medium leading-5 text-gray-800 hover:bg-gray-200"}">${escape(capitalizeFirstLetter(item))}
				</span>`;
  })}</div>` : ``}
