...
const MenuCell = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.icon === void 0 && $$bindings.icon && icon !== void 0)
    $$bindings.icon(icon);
  return `<div class="${"flex w-full flex-row items-center justify-between transition-all delay-75 ease-in-out hover:-rotate-1"}"><a class="${"delay-50 flex w-full items-center justify-between text-start text-2xl font-medium tracking-tight text-gray-900 transition-all hover:underline dark:text-white"}"${add_attribute("href", href, 0)}>${escape(name)}</a>

	<div>${validate_component(Icon, "Icon").$$render($$result, { width: "24", height: "24", name: icon }, {}, {})}</div></div>`;
});
