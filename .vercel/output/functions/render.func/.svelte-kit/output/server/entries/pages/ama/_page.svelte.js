import { c as create_ssr_component, a as escape, d as add_attribute, b as subscribe, e as each, v as validate_component } from "../../../chunks/index3.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-uh0d0z_START -->${$$result.title = `<title>Ama | Spikey Sanju</title>`, ""}<meta name="${"description"}" content="${"Ask me anything."}"><!-- HEAD_svelte-uh0d0z_END -->`, ""}

<section class="${"min-h-screen"}"><h1>Ask me anything</h1>
	<p class="${"w-full text-gray-500"}">I&#39;ll try to answer as many questions as possible.</p>

	${$page.data.session && $page.data.session.user ? `<button class="${"mt-3 rounded-sm border border-gray-200 bg-gray-100 px-3 py-2 font-medium text-black shadow-sm hover:cursor-pointer hover:bg-gray-200"}">Ask a question
		</button>` : `<button class="${"mt-3 rounded-sm border border-gray-200 bg-gray-100 px-3 py-2 font-medium text-black shadow-sm hover:cursor-pointer hover:bg-gray-200"}">Ask a question</button>`}
    ...
    return `<a${add_attribute("href", `/ama/${item.id}`, 0)}>${validate_component(AmaQuestionCard, "AmaCard").$$render(
      ...
      {}
		...
		${data.ama.length === 0 ? `<div class="${"flex h-full w-full flex-col items-center justify-center"}"><h2 class="${"text-2xl"}">No posts found.</h2>
				...
				<a href="${"/ama/new"}" class="${"btn btn-primary mt-4"}">Ask a question </a></div>` : ``}</div></section>`;
});
export {
  Page as default
};
