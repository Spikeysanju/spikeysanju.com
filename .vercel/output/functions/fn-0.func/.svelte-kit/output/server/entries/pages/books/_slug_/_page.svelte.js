...
<article><h1>${escape(data.title)}</h1>
    ...
    return `<a${add_attribute("href", `/books/categories/${item}`, 0)}><span class="${"inline-flex rounded-full bg-gray-100 px-2 text-xs font-medium leading-5 text-gray-800 hover:bg-gray-200"}">${escape(item)}</span>
				</a>`;
  })}</div>` : ``}
